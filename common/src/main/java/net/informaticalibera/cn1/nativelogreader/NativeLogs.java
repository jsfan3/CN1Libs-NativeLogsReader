...
public class NativeLogs {
    ...
    /**
     * Clear the native log of iOS and Android, remember to invoke
     * {@link #initNativeLogs() initNativeLogs()} in the init() of the main
     * class.
     *
     */
    public static void clearNativeLogs() {
        if (Display.getInstance().isSimulator()) {
            Log.p("No native logs in the Simulator...");
        } else {
            if (isInitialized && nativeLogsReader != null && nativeLogsReader.isSupported()) {
                nativeLogsReader.clearLog();
            } else {
                Log.p("clearNativeLogs() cannot be executed without calling initNativeLogs() before");
            }
        }
    }
}
